...
@make_m
async def help(app, message: Message):
    bot_config = await bot_db.get_bot_config()
    reply_markup = types.InlineKeyboardMarkup(
        [
            [
    ...
    await message.reply_text(bot_config["message"]["help_message"], reply_markup=reply_markup)
...
async def account(app, message: Message):
    ...
    await message.reply_text(await get_user_text(user))
